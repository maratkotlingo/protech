  ...
  <div class="space-y-5">
    <div
      ...
      <div class="relative overflow-hidden rounded-[1.65rem] bg-white dark:bg-zinc-950">
        ...
        <div
          class="pointer-events-none absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-zinc-950/20 to-transparent" />

        <div
          class="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1.5 text-xs font-medium text-zinc-600 shadow-lg shadow-zinc-950/10 backdrop-blur-xl dark:bg-zinc-950/75 dark:text-zinc-300">
    ...
    <div v-if="imageItems.length > 1" v-auto-animate class="flex gap-3 overflow-x-auto px-1 pb-2">
      <button v-for="(image, index) in imageItems" :key="image.url"
        ...
        <img :src="image.url" :alt="alt" class="size-20 rounded-xl object-cover sm:size-24" loading="lazy">
      </button>
    </div>

    <UModal v-model:open="zoomOpen" title="Фото товара" :ui="modalUi">
      <template #body>
