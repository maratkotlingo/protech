<template>
  <section class="space-y-8">
    ...
    <div class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.72fr)]">
      <div class="rounded-[2rem] bg-[#f9fafb] p-6 shadow-sm shadow-zinc-950/5 sm:p-8  ">
        <p class="text-sm font-semibold uppercase tracking-[0.24em] text-zinc-400">Описание</p>
        <h2 class="mt-3 text-3xl font-semibold tracking-normal text-zinc-950 ">
          История продукта
        </h2>
        <p class="mt-5 whitespace-pre-line text-lg leading-relaxed text-zinc-600 ">
          {{ product.description }}
        </p>
      </div>
      ...
      >
        <div
          ...
          :key="section.id"
          class="overflow-hidden rounded-[1.75rem] bg-[#f9fafb] shadow-sm shadow-zinc-950/5  "
        >
          <button
            type="button"
            class="flex w-full items-center justify-between gap-4 p-5 text-left transition duration-300 hover:bg-white/70 "
            @click="toggleInfoSection(section.id)"
          >
            <span class="flex items-center gap-3">
              <span class="grid size-10 place-items-center rounded-full bg-white text-emerald-600 shadow-sm shadow-zinc-950/5  ">
                <UIcon
                  :name="section.icon"
                  class="size-5"
                />
              </span>
              <span class="font-semibold text-zinc-950">{{ section.title }}</span>
            </span>
            <UIcon
          ...
          <div
            ...
            class="px-5 pb-5 text-sm leading-7 text-zinc-600 "
          >
            {{ section.body }}
          </div>
        </div>
