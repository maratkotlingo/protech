<template>
  <div class="mx-auto w-full max-w-370 px-4 py-5 sm:px-6 lg:px-8 lg:py-6">
    ...
    <OrderEmptyState
      ...
      description="После входа мы синхронизируем товары и сохраним корзину в вашем аккаунте."
      action-label="Войти"
      action-icon="i-lucide-user-round"
      action-to="/auth?redirect=/cart"
    />
    ...
    <OrderEmptyState
      ...
      title="Корзина пуста"
      description="Добавьте товары из каталога, чтобы собрать заказ."
    ...
    <div v-else class="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
      <section class="space-y-3">
        <div class="rounded-2xl bg-[#f9fafb] px-4 py-3 shadow-sm shadow-zinc-950/5">
          ...
          <UAlert
            v-if="hasBlockingItems"
            color="warning"
            variant="soft"
            icon="i-lucide-triangle-alert"
            class="mt-3 rounded-xl"
            title="Проверьте наличие"
